...
radius = r_p + h_alt
x, y, z, theta, phi = sphere_points(planet_centre[:3], radius, num=6)
...
for i in range(length_points):
    ...
    v_helio_ = circ_velocity_at_point_with_incl(points[i], planet_centre, i_deg, mu_earth)
    ...
    if v_helio_ is None or np.any(np.isnan(v_helio_)):
        ...
        v_sc_depart_short, v_sc_arrive_short = lt.lambert_solver(points[i], states_arrive[:3], tof, sun_mu, trajectory='pro')
...
# print(min(energies_valid))
# print(max(energies_valid))
...
# p = ax.scatter(points_valid[:,0], points_valid[:,1], points_valid[:,2], c=energies_valid, cmap='viridis', s=400)
# #Add colorbar
...
ax.scatter(
    ...
    color='black', s=50  # s controls dot size; increase for larger dots
)
...
ax.set_ylabel('Y [km]')
ax.set_zlabel('Z [km]')
ax.set_title(r"Energy dispersion of a Lambert transfer from Earth to Jupiter")
plt.show()
